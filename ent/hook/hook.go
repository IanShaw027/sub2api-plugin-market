...
// function as DownloadLog mutator.
type DownloadLogFunc func(context.Context, *ent.DownloadLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DownloadLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DownloadLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DownloadLogMutation", m)
}
...
// Mutate calls f(ctx, m).
func (f TrustKeyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TrustKeyMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TrustKeyMutation", m)
}
