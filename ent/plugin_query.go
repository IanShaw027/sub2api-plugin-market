...
func (_q *PluginQuery) QuerySubmissions() *SubmissionQuery {
...
// QueryDownloadLogs chains the current query on the "download_logs" edge.
func (_q *PluginQuery) QueryDownloadLogs() *DownloadLogQuery {
	query := (&DownloadLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PluginQuery) WithSubmissions(opts ...func(*SubmissionQuery)) *PluginQuery {
	query := (&SubmissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithDownloadLogs tells the query-builder to eager-load the nodes that are connected to
// the "download_logs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PluginQuery) WithDownloadLogs(opts ...func(*DownloadLogQuery)) *PluginQuery {
	query := (&DownloadLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PluginQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plugin, error) {
	...
	if query := _q.withSubmissions; query != nil {
		if err := _q.loadSubmissions(ctx, query, nodes,
			func(n *Plugin) { n.Edges.Submissions = []*Submission{} },
			func(n *Plugin, e *Submission) { n.Edges.Submissions = append(n.Edges.Submissions, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDownloadLogs; query != nil {
...
func (_q *PluginQuery) loadSubmissions(ctx context.Context, query *SubmissionQuery, nodes []*Plugin, init func(*Plugin), assign func(*Plugin, *Submission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Plugin)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(submission.FieldPluginID)
	}
	query.Where(predicate.Submission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plugin.SubmissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PluginQuery) loadDownloadLogs(ctx context.Context, query *DownloadLogQuery, nodes []*Plugin, init func(*Plugin), assign func(*Plugin, *DownloadLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Plugin)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(downloadlog.FieldPluginID)
	}
