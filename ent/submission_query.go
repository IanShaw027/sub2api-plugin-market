...
func (_q *SubmissionQuery) QueryPlugin() *PluginQuery {
...
func (_q *SubmissionQuery) QueryVersion() *PluginVersionQuery {
...
func (_q *SubmissionQuery) WithPlugin(opts ...func(*PluginQuery)) *SubmissionQuery {
	query := (&PluginClient{config: _q.config}).Query()
...
func (_q *SubmissionQuery) WithVersion(opts ...func(*PluginVersionQuery)) *SubmissionQuery {
	query := (&PluginVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(submission.FieldPluginID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Submission.Query().
//		Select(submission.FieldPluginID).
//		Scan(ctx, &v)
...
func (_q *SubmissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Submission, error) {
	...
	if query := _q.withPlugin; query != nil {
		if err := _q.loadPlugin(ctx, query, nodes, nil,
			func(n *Submission, e *Plugin) { n.Edges.Plugin = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVersion; query != nil {
		if err := _q.loadVersion(ctx, query, nodes, nil,
			func(n *Submission, e *PluginVersion) { n.Edges.Version = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubmissionQuery) loadPlugin(ctx context.Context, query *PluginQuery, nodes []*Submission, init func(*Submission), assign func(*Submission, *Plugin)) error {
...
func (_q *SubmissionQuery) loadVersion(ctx context.Context, query *PluginVersionQuery, nodes []*Submission, init func(*Submission), assign func(*Submission, *PluginVersion)) error {
	...
	query.Where(predicate.PluginVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(submission.VersionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.submission_version
		if fk == nil {
			return fmt.Errorf(`foreign-key "submission_version" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "submission_version" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SubmissionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPlugin != nil {
			_spec.Node.AddColumnOnce(submission.FieldPluginID)
		}
	}
